...
export const customThemeNames = MOBILE_THEME_IDS.flatMap((themeId) =>
  APPEARANCES.map((appearance) => `${themeId}-${appearance}`),
);

const variablesFor = (themeId: MobileThemeId, appearance: MobileThemeAppearance) => ({
...
const renderVariant = (name: string, themeId: MobileThemeId, appearance: MobileThemeAppearance) => {
