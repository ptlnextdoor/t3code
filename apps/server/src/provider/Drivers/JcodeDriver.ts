/**
 * JcodeDriver — `ProviderDriver` for the jcode agent (`jcode acp`) runtime.
 *
 * jcode exposes an ACP-based CLI like Cursor, but it is a local agent binary:
 * no OAuth, no API key, empty `authMethods`. Model catalog is discovered from
 * the `model` config option during the ACP handshake in the provider status
 * check. Text generation drives `runtime.prompt` and collects the
...
const UPDATE: ProviderMaintenanceCapabilitiesResolver = {
  resolve: (options) =>
    makeProviderMaintenanceCapabilities({
      ...
      packageName: null,
      updateExecutable: options?.binaryPath?.trim() || "jcode",
      updateArgs: ["update"],
      updateLockKey: "jcode",
    }),
};
